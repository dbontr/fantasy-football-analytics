...
The UI uses one dark-navy sidebar, a lightweight utility header, and a single shared component system across every workflow: 12px controls, 18px cards, 22px major panels, soft blue-white surfaces, restrained shadows, and progressive disclosure. My Team no longer duplicates the tool navigation already present in the sidebar, and Trade Ideas behaves as its own sidebar destination instead of appearing by default inside My Team. Browser QA audits every major page for legacy sharp card surfaces and the retired warm/cream theme, alongside desktop/tablet/mobile overflow checks. Technical/model controls remain secondary. Play-caller identity and offensive tendency metadata are now retained explicitly in coaching context and surfaced in Player Analysis, but direct points adjustments remain disabled until they earn their own prospective validation gate.
