...
SnapCount is public-first. The global navigation is **Home, Rankings, Players, Trade Analyzer, and My League**. Rankings, player analysis, projections/context, and the direct Trade Analyzer work immediately without configuring a fantasy platform or league. **My League requires an ESPN Fantasy league connection and a selected team before any personalized league tool unlocks.** Once connected, SnapCount imports the supported league rules, roster, opponents, schedule, and current week; league settings remain available as post-connection overrides.
...
Draft remains inside My League because its recommendations depend on league structure, so it now requires the same ESPN connection prerequisite as the other personalized league tools. Mock Draft automatically advances CPU picks between user turns; Live Draft Helper records the real room from any platform without inventing opponent picks. Technical/model controls remain hidden behind optional disclosures, and the major-sports-site visual system stays cool gray/white/near-black with sports-red decision accents.
