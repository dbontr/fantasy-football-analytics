...
SnapCount is public-first. The global navigation is **Home, Rankings, Players, Trade Analyzer, and My League**. Rankings, player analysis, projections/context, and the direct Trade Analyzer work immediately without configuring a fantasy platform or league. **No ESPN link or fantasy-platform account is required.** My League is the optional personalized layer: use one-click 12-team PPR defaults, customize scoring/lineup slots, or import league state to unlock Draft, Start / Sit, Waivers, Season Outlook, and league-aware trade context. ESPN remains an optional autofill adapter for supported rules, rosters, opponents, schedule, and current week.

The direct Trade Analyzer uses player typeahead fields rather than search-plus-select controls. Suggestions appear under the field while typing and are chosen directly. Each side starts with two player fields; when every visible field is filled, SnapCount adds another, so 3-for-2, 4-for-4, and larger packages can be analyzed without an arbitrary UI cap. Public mode searches the full player pool. When a real league roster is loaded, the user chooses the trade partner first; the give side is restricted to the user's roster and the receive side to that partner's roster. The existing qualified trade math and opponent-aware future-win layer consume the complete selected package.

Draft remains inside My League because its recommendations depend on league structure, but it still does not require an ESPN connection. Mock Draft automatically advances CPU picks between user turns; Live Draft Helper records the real room from any platform without inventing opponent picks. Technical/model controls remain hidden behind optional disclosures, and the major-sports-site visual system stays cool gray/white/near-black with sports-red decision accents.
...
- Platform-neutral league profiles: manual team count, PPR/Half-PPR/Standard scoring, QB/RB/WR/TE/FLEX/SUPERFLEX/DST/K/bench slots, local persistence, and shared propagation into lineup/trade/waiver/league simulation. ESPN can autofill supported settings; unsupported IDP or known custom offensive scoring is flagged instead of silently approximated.
...
- Interactive draft simulator + live draft helper: in Mock Draft the user controls one team while CPU opponents use configurable market/value/need/positional strategies and automatically advance between user turns; Live Draft Helper records the real room from any platform and never invents opponent picks. PPR 10/12-team standard-lineup Draft remains the A+-qualified scope; Half-PPR, Standard, Superflex, 2QB, other team counts, and custom slot structures are usable transfer-policy modes until separately qualified.
...
- Waiver add/drop search with waiver-priority/free-agency recommendations by default and bounded FAAB ranges only when FAAB mode is selected. In connected leagues, every recognized rostered player across all teams is excluded from the available-player pool; manual mode excludes the user's roster and does not pretend to know platform availability it has not imported.
- Direct trade analysis supports arbitrarily large user-entered packages; automated trade-idea search remains bounded to 1-for-1 and 2-for-2 candidates for tractable opponent search.
