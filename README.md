...
The product intentionally hides research terminology by default. The seven user-facing areas are **Home, Draft, Start / Sit, Trades, Waivers, Players, and Season**. **No ESPN link or fantasy-platform account is required.** Home starts with a platform-neutral league profile for team count, PPR/Half-PPR/Standard scoring, and lineup slots; ESPN is an optional autofill adapter for supported league rules, rosters, opponents, schedule, and current week. Yahoo, Sleeper, NFL, CBS, and custom-league users can use the manual tools directly. Draft includes the SnapCount Big Board, a guided mock room that automatically advances CPU picks after the user drafts, and a platform-agnostic live-draft helper that records real picks without inventing them. Trades includes a direct give/get analyzer; detailed distributions, model diagnostics, and custom what-if controls are kept behind optional disclosures when they are useful.

The interface uses a major-sports-site fantasy-desk layout rather than a boutique app shell. A two-row masthead keeps league/team/week context visible across every tool. Home is a real editorial dashboard: a main weekly-decision feed and universal league setup on the left, with ESPN presented second as an optional autofill path; Quick Tools and league context remain in a sticky right rail. On phones that collapses into one feed and the connected actions become a visible 2x2 grid. Technical runtime/data details are intentionally moved behind a disclosure. The visual system remains cool light gray, white editorial modules, near-black navigation and strong sports-red actions without copying ESPN's logo or exact interface. SnapCount never asks for ESPN passwords, SWID/`espn_s2` values, or cookie values. Anonymous ESPN access is attempted first; for a private league the user can explicitly opt into a direct browser-session request. In that mode the browser sends its existing ESPN session straight to ESPN, while SnapCount cannot read or persist the cookie itself.
