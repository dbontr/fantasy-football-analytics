...
SnapCount is public-first and uses a **two-tier navigation shell**: a compact global rail plus a clean contextual tool sidebar. The league switcher stays at the top of that sidebar instead of interrupting the menu. Before sync, the navigation contains only the basic public tools. After an ESPN league and team are connected, the same navigation upgrades in place: Mock Draft becomes Draft Assistant, Trade Analyzer opens league impact by default, and My Team, Start / Sit, Trade Ideas, Waiver Wire, and Season Outlook appear in their natural sections instead of being appended as a second locked menu. The connected league ID/team selection and personal player outlooks persist locally on the device.
