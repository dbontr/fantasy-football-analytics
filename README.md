...
SnapCount is public-first and uses a **left-sidebar product shell** rather than a crowded top navigation. Public tools — Home, Rankings, Player Analysis, My Outlooks, Mock Draft, and Basic Trade Value — work without connecting a fantasy platform. A single **League Sync** card explains the upgrade path. After an ESPN league and team are connected, the sidebar unlocks Draft Assistant, Start / Sit, Advanced Trade Lab, Waiver Wire, Trade Ideas, and Season Outlook. The connected league ID/team selection and personal player outlooks persist locally on the device.
...
My Outlooks lets a user record a bounded personal view — Super high, Positive, Somewhat positive, Neutral, Somewhat negative, Negative, or Unknown. **Unknown is distinct from Neutral**: Unknown means no opinion has been expressed, while Neutral is an explicit reviewed choice. Outlooks are persisted locally and only add a small close-call overlay to the qualified Draft score; roster construction, market timing, positional scarcity, availability, and the frozen base policy remain in control.

Mock Draft remains a public tool with a default 12-team PPR setup, a persistent team-by-team room board, and strategy-aware recommendations. My League Draft Center offers Live Draft Assistant or a league-prefilled launch into the same Mock Draft simulator. The Home page also exposes a descriptive cross-site historical benchmark using archived FantasyData, Fantasy Football Calculator, and MyFantasyLeague ADP sources on the frozen 2018 holdout; the chart is labeled retrospective evidence rather than a future-performance guarantee.

The UI uses a restrained blue/white sports-tool visual system with progressive disclosure, fewer simultaneous controls, and a mobile sidebar drawer. Technical/model controls remain secondary. Play-caller identity and offensive tendency metadata are now retained explicitly in coaching context and surfaced in Player Analysis, but direct points adjustments remain disabled until they earn their own prospective validation gate.
