...
The visual system is a major-sports-site-inspired fantasy desk rather than a boutique app theme: cool light-gray canvas, white editorial modules, a near-black navigation bar, strong sports-red actions and section rules, compact ranking/scorecard tables, restrained position colors, and an ESPN-red provider badge only where ESPN itself is involved. It borrows the visual language of sports coverage without copying ESPN's logo or exact interface. The product voice stays direct and beginner-friendly but varies by task: draft strategy, lineup certainty, trade judgment, waiver opportunity, and player scouting each have distinct language. SnapCount never asks for ESPN passwords, SWID/`espn_s2` values, or cookie values. Anonymous ESPN access is attempted first; for a private league the user can explicitly opt into a direct browser-session request. In that mode the browser sends its existing ESPN session straight to ESPN, while SnapCount cannot read or persist the cookie itself.
