...
Mock Draft is a public top-level tool with a clean default 12-team PPR setup; one **Start mock** action advances CPU opponents to the user, every user pick automatically advances the room again, and a persistent team-by-team board shows who every simulated team selected. My League Draft Center now starts with two explicit choices instead of forcing a mock: **Live Draft Assistant** tracks each real pick and continuously refreshes strategy-aware targets, while **Mock draft** opens the normal public simulator with the connected league's team count, scoring, lineup structure, rounds, and draft spot prefilled. Live recommendations expose roster needs, market price, value-over-next-alternative/wait cost, positional run pressure, and probability a player survives to the user's next turn; they are not a pure best-player-available list. Big Board, custom market board, and strategy benchmark remain secondary disclosures. Technical/model controls stay hidden behind optional disclosures, and the major-sports-site visual system stays cool gray/white/near-black with sports-red decision accents.
