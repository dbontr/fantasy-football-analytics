...
SnapCount is public-first. The global navigation is **Home, Rankings, Players, Mock Draft, Trade Analyzer, plus a My League dropdown**. Rankings, player analysis, projections/context, the direct Trade Analyzer, and a full CPU Mock Draft work immediately without configuring a fantasy platform or league. **My League requires an ESPN Fantasy league connection and a selected team before personalized league tools unlock.** Once connected, SnapCount imports supported league rules, roster, opponents, schedule, and current week; the dropdown then opens the league command center, contextual Draft Center, Start / Sit, trade ideas, Waivers, and Season Outlook.
...
Mock Draft is now a public top-level tool with a clean default 12-team PPR setup; one **Start mock** action advances CPU opponents to the user, and every user pick automatically advances the room again. My League keeps a separate contextual Draft Center that reuses the same engine with ESPN league size/scoring/lineup settings already applied and adds the Live Draft Helper. Big Board, room history, custom market board, and strategy benchmark live behind disclosures instead of filling the primary draft screen. Technical/model controls remain hidden behind optional disclosures, and the major-sports-site visual system stays cool gray/white/near-black with sports-red decision accents.
...
- Public Mock Draft + My League Draft Center: the public simulator defaults to 12-team PPR and works without ESPN; CPU opponents use configurable market/value/need/positional strategies and automatically advance between user turns. The My League version applies the connected ESPN format and exposes the Live Draft Helper, which records the real room from any platform and never invents opponent picks. PPR 10/12-team standard-lineup Draft remains the A+-qualified scope; Half-PPR, Standard, Superflex, 2QB, other team counts, and custom slot structures are usable transfer-policy modes until separately qualified.
...
- Direct Trade Analyzer supports arbitrarily large public user-entered packages. The separate automated trade-idea search lives only inside connected My League and remains bounded to 1-for-1 and 2-for-2 candidates for tractable opponent search.
