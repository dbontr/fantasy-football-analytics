  ...
  function coachingEvidence(player, profile) {
    ...
    return {
      "coaching.staff_context": {
        ...
        source: "staff and play-caller context retained; direct mean effect disabled pending walk-forward validation",
        ...
        tendencies: profile.offense ? { pace: finite(profile.offense.pace, 0.5), passRate: finite(profile.offense.passRate, 0.5), targetConcentration: finite(profile.offense.targetConcentration, 0.5), rbCommittee: finite(profile.offense.rbCommittee, 0.5), teUsage: finite(profile.offense.teUsage, 0.5), qbRun: finite(profile.offense.qbRun, 0.5), playAction: finite(profile.offense.playAction, 0.5), motion: finite(profile.offense.motion, 0.5), redZone: finite(profile.offense.redZone, 0.5) } : null,
        newStaff: Boolean(profile.newStaff),
      },
      "coaching.playcaller_context": {
        available: Boolean(profile.offensivePlayCaller || profile.offensiveCoordinator || profile.headCoach),
        value: 0,
        confidence: clamp(profile.confidence, 0.2, 0.9),
        conflict: profile.newStaff ? 0.18 : 0.05,
        model: "context-only-playcaller-profile",
        source: "named offensive play caller retained as context; no direct points adjustment without prospective validation",
        playCaller: profile.offensivePlayCaller || profile.offensiveCoordinator || profile.headCoach || null,
        coordinator: profile.offensiveCoordinator || null,
