(function attachSnapCountFootballContext(root, factory) {
  ...
  function roleUncertaintyAdjustment(evidence = {}) {
    const camp = evidence?.["role.camp_state"];
    if (!camp || camp.available === false) return { roleDelta: 0, availabilityDelta: 0, reason: null };
    const confidence = clamp(camp.confidence, 0, 0.6);
    const conflict = clamp(camp.conflict, 0, 1);
    const structuralMagnitude = Math.abs(clamp(camp.roleScore, -1, 1));
