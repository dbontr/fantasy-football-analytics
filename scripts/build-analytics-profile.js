...
function main() {
  ...
  requireGate(draftQualified(draft), "draft market superiority / synthetic-control non-inferiority");

  const datasetPath = path.join(validationDir, "historical-ppr-2020-2025.json.gz");
  const reportNames = ["forecast-audit-report.json", "uncertainty-audit-report.json", "decision-audit-report.json", "waiver-audit-report.json", "trade-audit-report.json", "season-audit-report.json", "draft-segmented-policy.json"];
  ...
  const qualification = {
    version: "snapcount-analytics-qualification-2026.1",
    qualifiedAt,
    ...
    grades: {
      ...
      waivers: "A+", trades: "A+", draft: "A+", season: "A+", provenance: "A+", runtimeParity: "A+",
    },
    evidence: {
      ...
      draftFresh2020: draft.fresh2020, draftConsistency2024: draft.consistency2024, draftConsistency2025: draft.consistency2025,
    },
  };

  const runtimeProfile = {
    version: "snapcount-runtime-profile-2026.1",
    qualifiedAt,
    ...
    draft: { policy: "segmented-qualified", supportedTeamCounts: [10, 12], segments: draftSegments, fallbackPolicy: draftSim.DEFAULT_ORACLE_POLICY },
  ...
  console.log("All serving surfaces: A+");
}
