...
async function main() {
  ...
  await waitFor(`document.querySelectorAll('#outlook-table tr[data-snap-rank]').length >= 100`, 10000);
  const outlookBoardShell = await evaluate(`({teams:document.querySelector('#outlook-teams')?.value||'',disabled:Boolean(document.querySelector('#outlook-teams')?.disabled),source:document.querySelector('#outlook-team-source')?.textContent||'',sort:document.querySelector('#outlook-sort')?.value||'',dividers:Boolean(document.querySelector('#outlook-round-dividers')?.checked),guide:document.querySelector('#outlook-round-guide')?.textContent||'',rounds:document.querySelectorAll('#outlook-table .outlook-round-row').length,firstRound:document.querySelector('#outlook-table .outlook-round-row')?.textContent||'',headers:[...document.querySelectorAll('#outlooks thead th')].map(node=>node.textContent.trim()),adjusted:document.querySelectorAll('.outlook-adjusted-rank,.outlook-change,[data-player-pass],.outlook-pass-badge').length})`);
  if (outlookBoardShell.teams !== '12' || outlookBoardShell.disabled || !outlookBoardShell.source.includes('manual') || outlookBoardShell.sort !== 'snap' || !outlookBoardShell.dividers || outlookBoardShell.rounds < 8 || !outlookBoardShell.firstRound.includes('Picks 1') || !outlookBoardShell.firstRound.includes('12') || outlookBoardShell.adjusted !== 0 || JSON.stringify(outlookBoardShell.headers) !== JSON.stringify(['SnapCount','Player','ESPN','ESPN gap','Outlook note'])) throw new Error(`Outlook notes-only board shell failed: ${JSON.stringify(outlookBoardShell)}`);
  ...
  if (outlookToolbarAlignment.labels.length !== 5 || outlookToolbarAlignment.controls.length !== 5 || outlookToolbarAlignment.labelSpread > 1 || outlookToolbarAlignment.controlSpread > 1) throw new Error(`Outlook toolbar fields are vertically misaligned: ${JSON.stringify(outlookToolbarAlignment)}`);
  const outlookNoteOptions = await evaluate(`[...document.querySelector('#outlook-table [data-player-outlook]').options].map(option=>({value:option.value,text:option.textContent}))`);
  if (JSON.stringify(outlookNoteOptions) !== JSON.stringify([{value:'',text:'No note'},{value:'love',text:'Love'},{value:'like',text:'Like'},{value:'neutral',text:'Neutral'},{value:'concerned',text:'Concerned'},{value:'fade',text:'Fade'}])) throw new Error(`Outlook note choices are not the five-note scale: ${JSON.stringify(outlookNoteOptions)}`);
  const snapOrderBeforeNote = await evaluate(`[...document.querySelectorAll('#outlook-table tr[data-snap-rank]')].slice(0,30).map(row=>row.dataset.snapRank+':'+row.querySelector('.player-identity-copy strong')?.textContent)`);
  const outlookValueSignals = await evaluate(`(() => { const rows=[...document.querySelectorAll('#outlook-table tr[data-snap-rank]')].slice(0,100); const values=rows.map((row)=>({signal:row.dataset.valueSignal,edge:Number(row.dataset.valueEdge),threshold:Number(row.dataset.valueThreshold),espn:Number(row.dataset.espnRank),snap:Number(row.dataset.snapRank)})); return {rows:values.length,valid:values.every((row)=>{if(row.espn>=9999)return row.signal==='unknown';const expected=row.edge>=row.threshold?'value':row.edge<=-row.threshold?'early':'close';return row.edge===Math.round(row.espn-row.snap)&&row.signal===expected;}),value:values.filter(row=>row.signal==='value').length,early:values.filter(row=>row.signal==='early').length,close:values.filter(row=>row.signal==='close').length}; })()`);
  if (outlookValueSignals.rows < 50 || !outlookValueSignals.valid) throw new Error(`Outlook ESPN-vs-SnapCount gap failed: ${JSON.stringify(outlookValueSignals)}`);
  await evaluate(`(() => { const select=document.querySelector('#outlook-sort'); select.value='espn'; select.dispatchEvent(new Event('change',{bubbles:true})); return true; })()`);
  await waitFor(`document.querySelector('#outlook-count')?.textContent.includes('ESPN order')`, 5000);
  const espnSortedOutlooks = await evaluate(`(() => { const rows=[...document.querySelectorAll('#outlook-table tr[data-snap-rank]')].slice(0,80); const ranks=rows.map(row=>Number(row.dataset.espnRank)); return {ascending:ranks.every((rank,index)=>index===0||rank>=ranks[index-1]),firstRound:document.querySelector('#outlook-table .outlook-round-row')?.textContent||'',guide:document.querySelector('#outlook-round-guide')?.textContent||''}; })()`);
  if (!espnSortedOutlooks.ascending || !espnSortedOutlooks.firstRound.includes('ESPN order') || !espnSortedOutlooks.guide.includes('ESPN rank')) throw new Error(`ESPN Outlook sort failed: ${JSON.stringify(espnSortedOutlooks)}`);
  await evaluate(`(() => { const box=document.querySelector('#outlook-round-dividers'); box.checked=false; box.dispatchEvent(new Event('change',{bubbles:true})); return true; })()`);
  await waitFor(`document.querySelector('#outlook-count')?.textContent.includes('dividers off') && document.querySelectorAll('#outlook-table .outlook-round-row').length===0`, 5000);
  await evaluate(`(() => { const box=document.querySelector('#outlook-round-dividers'); box.checked=true; box.dispatchEvent(new Event('change',{bubbles:true})); const select=document.querySelector('#outlook-sort'); select.value='snap'; select.dispatchEvent(new Event('change',{bubbles:true})); return true; })()`);
  await waitFor(`document.querySelector('#outlook-count')?.textContent.includes('SnapCount order') && document.querySelectorAll('#outlook-table .outlook-round-row').length>0`, 5000);
  const setOutlook = async (name, value) => {
    const changed = await evaluate(`(() => { const row=[...document.querySelectorAll('#outlook-table tr[data-snap-rank]')].find(r=>r.textContent.includes(${JSON.stringify(name)})); const select=row?.querySelector('[data-player-outlook]'); if(!select)return false; select.value=${JSON.stringify(value)}; select.dispatchEvent(new Event('change',{bubbles:true})); return true; })()`);
    if (!changed) throw new Error(`Could not set Outlook note for ${name}`);
    await waitFor(`[...document.querySelectorAll('#outlook-table tr[data-snap-rank]')].some(r=>r.textContent.includes(${JSON.stringify(name)})&&r.dataset.outlookKey===${JSON.stringify(value)})`, 5000);
  };
  await setOutlook('Jahmyr Gibbs','love');
  await setOutlook('Puka Nacua','neutral');
  const snapOrderAfterNote = await evaluate(`[...document.querySelectorAll('#outlook-table tr[data-snap-rank]')].slice(0,30).map(row=>row.dataset.snapRank+':'+row.querySelector('.player-identity-copy strong')?.textContent)`);
  if (JSON.stringify(snapOrderBeforeNote) !== JSON.stringify(snapOrderAfterNote)) throw new Error('Outlook note changed the canonical SnapCount order');
  ...
  await setOutlook('Brandon Aubrey','fade');
  ...
  await waitFor(`document.querySelectorAll('#outlook-table tr[data-snap-rank]').length === 3`, 5000);
  const outlookState = await evaluate(`(() => { const rows=[...document.querySelectorAll('#outlook-table tr[data-snap-rank]')]; return {text:document.querySelector('#outlook-table')?.textContent||'',values:[...document.querySelectorAll('#outlook-table [data-player-outlook]')].map(s=>s.value),playerMedia:document.querySelectorAll('#outlook-table .player-media').length,teamLogos:document.querySelectorAll('#outlook-table .player-team-logo').length,noteOnlyCopy:document.querySelector('#outlooks .section-heading')?.textContent||'',explainer:document.querySelector('#outlooks .outlook-explainer')?.textContent||'',rowBasis:document.querySelectorAll('#outlook-table .outlook-basis').length,forbidden:document.querySelectorAll('.outlook-adjusted-rank,.outlook-change,[data-player-pass],.outlook-pass-badge').length}; })()`);
  if (!outlookState.text.includes('Jahmyr Gibbs') || !outlookState.text.includes('Puka Nacua') || !outlookState.text.includes('Brandon Aubrey') || !outlookState.values.includes('love') || !outlookState.values.includes('neutral') || !outlookState.values.includes('fade') || outlookState.playerMedia < 3 || outlookState.teamLogos < 3 || !outlookState.noteOnlyCopy.includes('private notes only') || !outlookState.explainer.includes('never move a player') || outlookState.rowBasis !== 0 || outlookState.forbidden !== 0) throw new Error(`Outlook notes-only state failed: ${JSON.stringify(outlookState)}`);
  ...
  const outlookCellContinuity = await evaluate(`(() => { const row=document.querySelector('#outlook-table tr[data-snap-rank]'); const cells=row ? [...row.children] : []; if(cells.length < 2) return {missing:true}; const first=cells[0].getBoundingClientRect(), second=cells[1].getBoundingClientRect(); return {missing:false,display:getComputedStyle(cells[0]).display,gap:Number((second.left-first.right).toFixed(2))}; })()`);
  ...
  const outlookDraftGuard = await evaluate(`(() => { const rows=[...document.querySelectorAll('#draft-table tr')]; const gibbs=rows.find(r=>r.textContent.includes('Jahmyr Gibbs')); const bigGibbs=[...document.querySelectorAll('#draft-big-board .big-board-row')].find(r=>r.textContent.includes('Jahmyr Gibbs')); return {identityOrder:rows.every(r=>Number(r.dataset.qualifiedRank)===Number(r.dataset.personalRank)),legacyShiftAttrs:rows.filter(r=>r.hasAttribute('data-outlook-shift')).length,gibbsNote:gibbs?.dataset.outlookNote||'',gibbsText:gibbs?.textContent||'',gibbsTone:gibbs?.querySelector('.outlook-chip')?.className||'',gibbsTitle:gibbs?.querySelector('.outlook-chip')?.title||'',playerMedia:document.querySelectorAll('#draft-table .player-media').length,teamLogos:document.querySelectorAll('#draft-table .player-team-logo').length,bigBoardText:bigGibbs?.textContent||'',bigBoardTone:bigGibbs?.querySelector('.outlook-chip')?.className||'',passArtifacts:document.querySelectorAll('#draft [data-player-pass],#draft .outlook-pass-badge,.big-board-row.is-pass').length}; })()`);
  if (!outlookDraftGuard.identityOrder || outlookDraftGuard.legacyShiftAttrs !== 0 || outlookDraftGuard.gibbsNote !== 'love' || !outlookDraftGuard.gibbsText.includes('NOTE · Love') || !outlookDraftGuard.gibbsText.includes('ESPN RB1') || !outlookDraftGuard.gibbsText.includes('SNAP RB1') || !outlookDraftGuard.gibbsTone.includes('love') || !outlookDraftGuard.gibbsTitle.includes('does not affect') || outlookDraftGuard.playerMedia < 10 || outlookDraftGuard.teamLogos < 10 || !outlookDraftGuard.bigBoardText.includes('NOTE · Love') || !outlookDraftGuard.bigBoardTone.includes('love') || outlookDraftGuard.passArtifacts !== 0) throw new Error(`Outlook note changed or polluted the qualified Draft order: ${JSON.stringify(outlookDraftGuard)}`);
  ...
  await waitFor(`!document.querySelector('#league-command-strip').classList.contains('hidden') && document.querySelectorAll('#roster-strip .roster-chip').length===9`, 10000);
  ...
  if (espnSync.team !== 'QA Champions' || espnSync.league !== 'QA Sunday League' || espnSync.roster !== 9 || espnSync.week !== '3' || espnSync.title !== 'QA Champions' || espnSync.waiverMode !== 'priority') throw new Error(`ESPN league sync/default waiver flow failed: ${JSON.stringify(espnSync)}`);
  ...
  if (myTeamUi.rail !== 'none' || myTeamUi.commands === 'none' || myTeamUi.dashboard === 'none' || myTeamUi.radius < 12 || myTeamUi.rosterRows !== 9 || !myTeamUi.rosterCount.includes('9 recognized') || !myTeamUi.matchup.includes('QA Rivals') || !myTeamUi.waivers || myTeamDesktop.horizontalOverflow) throw new Error(`My Team dashboard failed: ${JSON.stringify({myTeamUi,myTeamDesktop})}`);
  const winPlanVisible = await evaluate(`!document.querySelector('#win-plan-card')?.classList.contains('hidden') && Boolean(document.querySelector('#run-win-plan'))`);
  if (!winPlanVisible) throw new Error('Connected Win Plan is not visible');
  await evaluate(`document.querySelector('#run-win-plan').click(); true`);
  await waitFor(`!document.querySelector('#run-win-plan')?.disabled && (document.querySelector('#win-plan-result')?.textContent.includes('Baseline remaining wins') || document.querySelector('#win-plan-result')?.textContent.includes('could not finish'))`, 90000);
  const winPlan = await evaluate(`({text:document.querySelector('#win-plan-result')?.textContent||'',disabled:Boolean(document.querySelector('#run-win-plan')?.disabled)})`);
  if (winPlan.disabled || !winPlan.text.includes('Baseline remaining wins') || !['HOLD','SET LINEUP','MAKE WAIVER MOVE','MAKE TRADE'].some((label)=>winPlan.text.includes(label))) throw new Error(`Connected Win Plan failed: ${JSON.stringify(winPlan)}`);
  const winPlanDesktop = await snapshot('win-plan-desktop', 1440, 1000);
  if (winPlanDesktop.horizontalOverflow) throw new Error('Win Plan desktop overflow');
  ...
  await waitFor(`document.querySelectorAll('#outlook-table tr[data-snap-rank]').length === 3`, 5000);
  const persistedOutlooks = await evaluate(`({text:document.querySelector('#outlook-table')?.textContent||'',values:[...document.querySelectorAll('#outlook-table [data-player-outlook]')].map((row)=>row.value),teams:document.querySelector('#outlook-teams')?.value||'',teamsDisabled:Boolean(document.querySelector('#outlook-teams')?.disabled),teamSource:document.querySelector('#outlook-team-source')?.textContent||'',sort:document.querySelector('#outlook-sort')?.value||'',dividers:Boolean(document.querySelector('#outlook-round-dividers')?.checked),guide:document.querySelector('#outlook-round-guide')?.textContent||''})`);
  if (!persistedOutlooks.text.includes('Jahmyr Gibbs') || !persistedOutlooks.text.includes('Puka Nacua') || !persistedOutlooks.text.includes('Brandon Aubrey') || !persistedOutlooks.values.includes('love') || !persistedOutlooks.values.includes('neutral') || !persistedOutlooks.values.includes('fade') || persistedOutlooks.teams !== '4' || !persistedOutlooks.teamsDisabled || !persistedOutlooks.teamSource.includes('synced') || persistedOutlooks.sort !== 'snap' || !persistedOutlooks.dividers || !persistedOutlooks.guide.includes('SnapCount rank') || !persistedOutlooks.guide.includes('Round 1')) throw new Error(`Player Outlook notes / synced round size did not persist: ${JSON.stringify(persistedOutlooks)}`);
