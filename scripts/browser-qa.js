...
async function main() {
  ...
  const kickerOutlook = await evaluate(`(() => { const row=[...document.querySelectorAll('#outlook-table tr')].find(r=>r.textContent.includes('Brandon Aubrey')); const select=row?.querySelector('[data-player-outlook]'); if(!select)return false; select.value='super-positive'; select.dispatchEvent(new Event('change',{bubbles:true})); return true; })()`);
  ...
  await waitFor(`document.querySelectorAll('#outlook-table tr').length === 3`, 5000);
  const outlookState = await evaluate(`(() => { const rows=[...document.querySelectorAll('#outlook-table tr')]; const gibbs=rows.find(r=>r.textContent.includes('Jahmyr Gibbs')); const aubrey=rows.find(r=>r.textContent.includes('Brandon Aubrey')); return {text:document.querySelector('#outlook-table')?.textContent||'',values:[...document.querySelectorAll('#outlook-table [data-player-outlook]')].map(s=>s.value),playerMedia:document.querySelectorAll('#outlook-table .player-media').length,teamLogos:document.querySelectorAll('#outlook-table .player-team-logo').length,rankChips:document.querySelectorAll('#outlook-table .rank-source-chip').length,gibbsBasis:gibbs?.querySelector('.outlook-basis')?.textContent||'',aubreyTone:aubrey?.querySelector('.outlook-chip')?.className||'',aubreyColor:aubrey?.querySelector('.outlook-chip')?getComputedStyle(aubrey.querySelector('.outlook-chip')).color:''}; })()`);
  if (!outlookState.text.includes('Jahmyr Gibbs') || !outlookState.text.includes('Puka Nacua') || !outlookState.text.includes('Brandon Aubrey') || !outlookState.values.includes('positive') || !outlookState.values.includes('neutral') || !outlookState.values.includes('super-positive') || outlookState.playerMedia < 3 || outlookState.teamLogos < 3 || outlookState.rankChips < 6 || !outlookState.gibbsBasis.includes('ESPN') || !outlookState.aubreyTone.includes('super-positive')) throw new Error(`Outlook state failed: ${JSON.stringify(outlookState)}`);
  ...
  if (espnSync.team !== 'QA Champions' || espnSync.league !== 'QA Sunday League' || espnSync.roster !== 3 || espnSync.week !== '3' || espnSync.title !== 'QA Champions') throw new Error("ESPN league sync flow failed");
  ...
  if (!persistedOutlooks.text.includes('Jahmyr Gibbs') || !persistedOutlooks.text.includes('Puka Nacua') || !persistedOutlooks.text.includes('Brandon Aubrey') || !persistedOutlooks.values.includes('positive') || !persistedOutlooks.values.includes('neutral') || !persistedOutlooks.values.includes('super-positive')) throw new Error(`Player outlooks did not persist across reload: ${JSON.stringify(persistedOutlooks)}`);
  ...
  const result = { home, homeStructure, outlookState, outlookDesktop, publicRankings, rankingsDesktop, publicDraftBefore, publicMock, publicDraftDesktop, publicTrade, publicTradeDesktop, noConnectionAccess, syncGateBefore, myLeagueBefore, myTeamDesktop, relockedLeague, postDisconnectTrade, postDisconnectMock, espnSync, connectedLeagueUi, connectedHome, persistedLeague, persistedOutlooks, player, veteran, liveNews, rookie, rookieTablet, leagueDraftShell, leagueMockPreset, mockFlow, board, benchmark, liveDraftStart, liveOnClock, draftRoom, draftDesktop, lineup, lineupDesktop, syncedBasicTrade, syncedBasicResult, tradePartnerControl, trade, thirdPartyShell, thirdPartyTrade, tradeDesktop, tradeIdeas, tradeIdeasDesktop, waivers, waiversDesktop, season, seasonDesktop, draftMobile, tradeMobile, homeMobile, drawerOpen, errors,
