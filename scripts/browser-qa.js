...
async function main() {
  ...
  const kickerOutlook = await evaluate(`(() => { const row=[...document.querySelectorAll('#outlook-table tr')].find(r=>r.textContent.includes('Brandon Aubrey')); const select=row?.querySelector('[data-player-outlook]'); if(!select)return false; select.value='super-high'; select.dispatchEvent(new Event('change',{bubbles:true})); return true; })()`);
  ...
  if (!outlookState.text.includes('Jahmyr Gibbs') || !outlookState.text.includes('Puka Nacua') || !outlookState.text.includes('Brandon Aubrey') || !outlookState.values.includes('positive') || !outlookState.values.includes('neutral') || !outlookState.values.includes('super-high')) throw new Error(`Outlook state failed: ${JSON.stringify(outlookState)}`);
  ...
  const outlookDraftGuard = await evaluate(`(() => { const rows=[...document.querySelectorAll('#draft-table tr')]; return {maxShift:Math.max(0,...rows.map(r=>Math.abs(Number(r.dataset.outlookShift||0)))),aubreyVisible:rows.some(r=>r.textContent.includes('Brandon Aubrey')),gibbs:rows.find(r=>r.textContent.includes('Jahmyr Gibbs'))?.dataset.outlookShift||'0'}; })()`);
  if (outlookDraftGuard.maxShift > 6.7 || outlookDraftGuard.aubreyVisible) throw new Error(`Timing-aware personal outlook guard failed: ${JSON.stringify(outlookDraftGuard)}`);
  ...
  if (!persistedOutlooks.text.includes('Jahmyr Gibbs') || !persistedOutlooks.text.includes('Puka Nacua') || !persistedOutlooks.text.includes('Brandon Aubrey') || !persistedOutlooks.values.includes('positive') || !persistedOutlooks.values.includes('neutral') || !persistedOutlooks.values.includes('super-high')) throw new Error(`Player outlooks did not persist across reload: ${JSON.stringify(persistedOutlooks)}`);
