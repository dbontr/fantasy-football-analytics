...
async function main() {
  ...
  const lockedRounds = await evaluate(`(() => { const rows=[...document.querySelectorAll('#outlook-table tr[data-personal-rank]')].slice(0,80); return {checked:Boolean(document.querySelector('#outlook-round-lock')?.checked),rows:rows.length,allInside:rows.every(r=>Math.ceil(Number(r.dataset.personalRank)/10)===Math.ceil(Number(r.dataset.espnRank)/10)),adjusted:document.querySelectorAll('#outlook-table .outlook-adjusted-rank').length,different:rows.filter(r=>Number(r.dataset.freeRank)!==Number(r.dataset.personalRank)).length}; })()`);
  if (!lockedRounds.checked || lockedRounds.rows < 50 || !lockedRounds.allInside || lockedRounds.adjusted < 50) throw new Error(`Round-locked Outlook sort failed: ${JSON.stringify(lockedRounds)}`);
