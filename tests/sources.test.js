...
test("PPR snapshot enrichment replaces mean projections but preserves local context", () => {
  ...
  const snapshot = { players: [{ player: { id: 1, proTeamId: 8, active: true, injuryStatus: "ACTIVE", ownership: { averageDraftPosition: 18.4, averageDraftPositionPercentChange: -1.2, auctionValueAverage: 31.5, percentOwned: 98.3, percentStarted: 82.1, date: 1786375822860 }, rankings: { "0": [{ rankSourceId: 0, rankType: "PPR", averageRank: 17.25 }] }, stats: [
  ...
  assert.equal(player.espnPlayerId, "1");
  assert.equal(player.espnTeamId, 8);
